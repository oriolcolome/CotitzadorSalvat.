...
with st.sidebar:
    ...
    st.info("Selecciona les dades per calcular la cotització.")
    st.caption("Versió Definitiva v7.0")

st.title("🚛 Calculadora d'Enviaments")

# --- CÀRREGA DE DADES (MODE DIRECTE) ---
@st.cache_data(ttl=600) # El 'ttl' refresca la memòria cada 10 minuts per evitar errors de connexió
def carregar_dades():
    fitxer = "dades.xlsx"
    
    # Comprovació de seguretat: El fitxer existeix?
    if not os.path.exists(fitxer):
        return None, None, None, f"NO TROBO EL FITXER '{fitxer}'. Assegura't que l'has pujat a GitHub amb aquest nom exacte."

    try:
        # 1. DADES GENERALS
        df_temp = pd.read_excel(fitxer, sheet_name="DATOS", header=None, nrows=20, engine='openpyxl')
        # Busquem la fila on posa 'PAISES'
        fila_pais = df_temp[df_temp.apply(lambda x: x.astype(str).str.contains('PAISES', case=False)).any(axis=1)].index[0]
        ...
        # Busquem la fila on posa 'ZIP CODE' o 'AUXILIAR'
        fila_tarifes = df_temp2[df_temp2.apply(lambda x: x.astype(str).str.contains('ZIP CODE|AUXILIAR', regex=True, case=False)).any(axis=1)].index[0]
        ...
        return df_datos, mapa, preus, None

    except Exception as e:
        return None, None, None, f"Error tècnic llegint l'Excel: {str(e)}"

# Executem la càrrega
df_datos, mapa_zones, df_preus, error_message = carregar_dades()

# GESTIÓ D'ERRORS VISUAL
if error_message:
    st.markdown(f"""
    <div class="error-card">
        ...
        <p>{error_message}</p>
        <hr>
        <p style="font-size:12px">Solució: Verifica que el fitxer 'dades.xlsx' està a GitHub i no està corrupte.</p>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
with c_right:
    if calcular:
        if not cp:
            st.error("⚠️ Introdueix un Codi Postal")
        else:
            ...
            if rutes.empty:
                ...
                if pes_tarifa and zona in df_preus.columns:
                    ...
                    # Càlculs suplements (MAUT, CITA, TASA)
